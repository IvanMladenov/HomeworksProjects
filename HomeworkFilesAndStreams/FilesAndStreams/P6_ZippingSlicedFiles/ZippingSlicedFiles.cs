...
namespace P5_SliceFile
{
    class SliceFile
    {
        ...
        static void Assemble(List<string> files, string destination)//Not finished to decompress!
        {
        ...
        static List<string> Slice(string sourseFile, string destinationDirectory, int parts)
        {
            ...
            using (FileStream sourse = new FileStream(sourseFile, FileMode.Open))
            {
                ...
                for (int i = 1; i <= parts; i++)
                {
                    ...
                    {
                        using(GZipStream zip=new GZipStream(create,CompressionMode.Compress,false))
                        {
